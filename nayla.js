...
const listMessage = {
text: `Anonymous chat, Temukan Teman dan temukan pasangan mu!`,
footer: "ANONYMOUS CHAT BOT WHATSAPP",
...
if (!args[0]) return nayla.sendMessage(from, { text: "[ *DAFTAR* ]\n\nPilih Jenis Kelamin", footer: 'Loading...', buttons: [{buttonId: `${prefix + command} x`, buttonText: {displayText: '[👦] Pria'}, type: 1}, {buttonId: `${prefix + command} y`, buttonText: {displayText: '[👧] Wanita'}, type: 1}],headerType: 1 })
if (args[0] == "x") {
daftar.push({id: sender, teman:false, gender: "pria", status:false})
fs.writeFileSync('./db/function/daftar.json', JSON.stringify(daftar))
reply(`[ *INFO KAMU* ]\n• *Status* : Sukses\n• *Id* : ${sender.split("@")[0]}\n• *User* : Free\n• *Gender* : Pria\n• *Teman* : false\n\n*Note* : Silahkan gunakan fitur menu untuk melihat apa saya fungsi bot ini`)}
if (args[0] == "y") {
...
if (isGroup) return reply("Gunakan bot ini di private message:3")
if (cekUser("id", sender) == null) return reply("Kamu belum terdaftar di database bot, Silahkan daftar terlebih dahulu, gunakan command #daftar")
...
if (budy == prefix + "sc"){reply("https://za.uy/4JGWB\nBase by Nayla chan><")} // JANGAN DI UBAH:(
}} catch (err) {
